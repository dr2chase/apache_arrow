...
func (timestampTraits) CastFromBytes(b []byte) []Timestamp {
	return unsafe.Slice((*Timestamp)(unsafe.Pointer(unsafe.SliceData(b))), cap(b)/TimestampSizeBytes)[:len(b)/TimestampSizeBytes]
}

// CastToBytes reinterprets the slice b to a slice of bytes.
func (timestampTraits) CastToBytes(b []Timestamp) []byte {
	return unsafe.Slice((*byte)(unsafe.Pointer(unsafe.SliceData(b))), cap(b)*TimestampSizeBytes)[:len(b)*TimestampSizeBytes]
}
